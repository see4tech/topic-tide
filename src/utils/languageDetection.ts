...
export const detectLanguage = (text: string): DetectionResult => {
  ...
  const englishWords = /\b(the|is|are|was|were|has|have|had|will|would|could|should|may|might|must|can|court|urges|block|says|new|report|update|man|with|to|backdoor|hackers|hacked|networks|pitch|services|cybersecurity|DOJ|and|by|about|stepping|up|its|bid|dethrone|getting|serious|verification|impersonation|what|going|on|with|name|uses|magazine|promote|software|billionaire|owner|time|launches|unveils|announces|reveals|introduces|releases|updates|improves|enhances|boosts|leverages|implements|deploys|integrates|incorporates|utilizes|adopts|embraces)\b/i;
  
  // Enhanced English patterns detection
  const englishPatterns = /\b(ing|ed|ly)\b|\b[A-Z][a-z]+ [A-Z][a-z]+\b|\b[A-Z][a-z]+\b|\b(AI|ChatGPT|X|DOJ|CEO|CTO|API|SDK|UI|UX|ML|AR|VR|IoT)\b/;
  ...
  // Enhanced force translate patterns for tech terms
  const forceTranslatePatterns = /(ChatGPT|Billionaire|AI Software|Magazine|Microsoft|Google|Apple|Meta|Twitter|X|Tesla|Amazon|OpenAI|Anthropic|Claude|Gemini|GitHub|Stack Overflow|Visual Studio|Code|Windows|Linux|MacOS|iOS|Android|AWS|Azure|GCP|API|SDK|Framework)\b/i;
